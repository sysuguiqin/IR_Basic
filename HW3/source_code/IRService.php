...
//====================================================文章分类===========Multinomial Model=========start================
    ...
    public function MBNB_classifier(){
        //获取全部的文件名单
        $origin_files_array = $this ->get_origin_files()[ "file_only_name"];
      ...
      //  $this->naive_bayes_prepare( );
       ...
       //  var_dump( $training_data_dictionary);
         ...
         $testing_process_data= $this ->mbnb_testing_process(  $training_data_dictionary,$training_data,$termsCF_array,$origin_files_array);
     ...
     * mbnb testing process
     * */
    public function mbnb_testing_process(  $training_data_dictionary,$training_data,$termsCF_array,$origin_files_array){
        //testing process
...
//====================================================文章分类===========Multinomial Model===========end================
  ...
  * */
    public function naive_bayes_prepare(){
        ...
        //获取全部的文件名单
        $origin_files_array = $this ->get_origin_files()[ "file_with_style"];
        //var_dump( $origin_files_array);
        ...
        //feature selection 选择出最具代表性的vocabulary
        $statistical_result = $this ->feature_selection( );
        //var_dump($statistical_result);
       ...
       //var_dump($termsCF_array);
        ...
        //计算并保存CF
       $termsDF_array= $this ->save_terms_CF_classifier($termsCF_array,$statistical_result,$termsDF_array);
    ...
    /*
       ...
       * */
    public  function save_terms_CF_classifier($termsCF_array,$dictionary,$termsDF_array){
        $termsCF_second_array =array();

        foreach($termsCF_array as $file_key => $file_values){
            if(!strstr( $file_key,"_terms_total")){
                $terms_count=0;
                foreach($file_values as $file_values_term_key =>  $file_values_term_value){
                  // var_dump($file_values_term_value);
                    if( in_array($file_values_term_value[0],$dictionary)){
                        $termsCF_second_array[$file_key][$file_values_term_value[0]][]=$file_values_term_value[0];
                        $termsCF_second_array[$file_key][$file_values_term_value[0]][]=$file_values_term_value[1];
                        $terms_count += 1;
                    }
                }
                $termsCF_second_array[$file_key][$file_key."_terms_total"][]=$file_key."_terms_total";
                $termsCF_second_array[$file_key][$file_key."_terms_total"][]=$terms_count;
            }
        }
        $file= "\\program_result\\tool_file\\doc_term_CF.txt";
        $type="w";
        $handle_file=fopen(dirname(__FILE__) .$file ,$type);

        foreach(  $termsCF_second_array as $key => $lines ){
            foreach( $lines as  $line){
                if(!strstr($line[0] ,"_terms_total")){
                    $line_content =$key." ".$line[0]." ".$line[1]."\r\n";
                    fwrite( $handle_file,$line_content);
                }else{
                    $line_content =$line[0]." ".$line[1]."\r\n";
                    fwrite( $handle_file,$line_content);
                }
            }
        }
        fclose( $handle_file);
        $termsDF_second_array = array();
        foreach($termsDF_array as $key => $value){
            if( in_array( $key,$dictionary,true)){
                $termsDF_second_array[$key]= $value;
            }
        }
       return $termsDF_second_array;
    }
//====================================================文章分类===========Feature Selection============start=============
    /*
    *Feature Selection likelihood ratios
    * */
    public function feature_selection( ){
        //获取training_data class-file 信息
        $training_data = $this->get_classifier_training_data();
        ...
        $statistical_table = $this ->feature_selection_prepare( $training_data, $termsCF_array);

        $class_terms_amount=39;
        $all_terms_amount=500;
        $statistical_result = $this ->log_likelihood_ratios( $class_terms_amount, $all_terms_amount,$training_data,$statistical_table );
  ...
  *Feature Selection likelihood ratios
  * */
    public function log_likelihood_ratios(  $class_terms_amount, $all_terms_amount,$training_data,$statistical_table ){
        $statistical_result =array();
        $files_training_amount=$statistical_table['training_data_files_amount'];
        //每一个class 选出40个 unique term
        foreach($training_data['class'] as $class ){
            foreach( $statistical_table['CLASS_'.$class] as $class_term_element ){
                $llr_value= -2*log(pow(($class_term_element[1]+$class_term_element[3])/ $files_training_amount,$class_term_element[1])
                        *pow((1-($class_term_element[1]+$class_term_element[3])/ $files_training_amount),$class_term_element[2])
                        *pow(($class_term_element[1]+$class_term_element[3])/ $files_training_amount,$class_term_element[3])
                        *pow((1-($class_term_element[1]+$class_term_element[3])/ $files_training_amount),$class_term_element[4]))
                    /(pow(($class_term_element[1])/ ($class_term_element[1]+$class_term_element[2]),$class_term_element[1])
                        *pow((1-($class_term_element[1]/($class_term_element[1]+$class_term_element[2]))),$class_term_element[2])
                        *pow(($class_term_element[3]/($class_term_element[3]+$class_term_element[4])),$class_term_element[3])
                        *pow((1-($class_term_element[3]/ ($class_term_element[3]+$class_term_element[4]))),$class_term_element[4]));

                $class_result[$class_term_element[0]] =$llr_value;
            }
            arsort( $class_result);
            $statistical_result[ $class] = $class_result;
        }
        //var_dump($statistical_result);
        $training_data_dictionary= array();
        $count=0;

        foreach($statistical_result as $element){
            $i=1;
            foreach($element as $key => $value){
                if($i <= $class_terms_amount){
                    if(@!in_array( $key,$training_data_dictionary,true)){
                        $count +=1;
                        if($count <= $all_terms_amount){
                            $i+=1;
                            $training_data_dictionary[]= $key;
                        }
                    }
                }
            }
    ...
    * */
    public function feature_selection_prepare( $training_data, $termsCF_array){
        $files_training_amount = count($training_data['file']);//training data 总的文章数量
        $dictionary = $this -> feature_selection_pre_dictionary($training_data,$termsCF_array);

        $class_all_array=$training_data['class'];
        $statistical_table = array();
        $statistical_table['training_data_files_amount']=$files_training_amount;
        foreach($class_all_array as $class_all_record){

            $on_topic=count($training_data['class_file'][$class_all_record]);
            foreach( $dictionary['CLASS_TERMS_'.$class_all_record] as $class_term){ //term present
                $on_topic_term_present =0;
                $off_topic_term_present =0;

                foreach($dictionary['TERM_FILES_'.$class_term] as $term_file){ //term persent
                    if(in_array( $term_file, $training_data['class_file'][$class_all_record],true)){
                        $on_topic_term_present +=1; //on topic term present
                    }else{
                        $off_topic_term_present +=1;//off topic term present
                    }
                }
                $on_topic_term_absent =$on_topic-$on_topic_term_present;
                $off_topic_term_absent=  $files_training_amount-$on_topic- $off_topic_term_present;
                $statistical_table['CLASS_'.$class_all_record]['CLASS_TERM_'. $class_term][]= $class_term;
                $statistical_table['CLASS_'.$class_all_record]['CLASS_TERM_'. $class_term][] = $on_topic_term_present;
                $statistical_table['CLASS_'.$class_all_record]['CLASS_TERM_'. $class_term][] = $on_topic_term_absent;
                $statistical_table['CLASS_'.$class_all_record]['CLASS_TERM_'. $class_term][] = $off_topic_term_present;
                $statistical_table['CLASS_'.$class_all_record]['CLASS_TERM_'. $class_term][] = $off_topic_term_absent;
            }
        }
        return $statistical_table;
    }
    /*
   *Feature Selection prepare return dictionary
   * */
    public function feature_selection_pre_dictionary($training_data,$termsCF_array){
        $dictionary =array();
        foreach($training_data['class'] as $specific_class){

            foreach($training_data['class_file'][$specific_class] as $specific_class_file){
                foreach($termsCF_array[$specific_class_file] as $key => $value ){
                    if(!@in_array(  $key,$dictionary["CLASS_TERMS_".$specific_class],true)){
                        $dictionary["CLASS_TERMS_".$specific_class][]= $key;
                        if(!@in_array(  $key,$dictionary["ALL_TERMS"],true)){
                            $dictionary["ALL_TERMS"][]= $key;
                        }
                    }
                    $dictionary["TERM_FILES_". $key][]=$specific_class_file;
                }
            }
        }
...
* */
    public function save_stopword_list(){
       ...
       // var_dump( $stopword_files_array); //读取所有stop-word list文件
        ...
        foreach( $stopword_files_array as $stopword_files ){
           ...
           //var_dump(  $file_path);
            ...
            if(file_exists($file_path)){
                ...
                //var_dump($lines);
                foreach ($lines as $line) {
                    if(!in_array( rtrim($line), $stopwords_english_array, true)){
        ...
        asort($stopwords_english_array);
        //var_dump($stopwords_english_array);
        foreach( $stopwords_english_array as $stopwords_english ){
            $line_content =rtrim( $stopwords_english)."\r\n";
...
public function  get_stopword_list(){
        //读取stopword_list
        $file_path =dirname(__FILE__)."\\stop_words"."\\stop_words_english.txt";
    ...
    public function get_terms_CF(){
        ...
        if(file_exists($file_path)){
            ...
            foreach($file_content as $line){
                ...
                //var_dump($line);
                if(!strstr($line[0],"_terms_total")){
                    @$doc_terms_CF[$line[0]][rtrim($line[1])][] = rtrim($line[1]);
                    @$doc_terms_CF[$line[0]][rtrim($line[1])][] = rtrim($line[2]);
                }else{
